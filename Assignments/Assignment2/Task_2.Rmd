...
output:
  pdf_document:
    ...
    extra_dependencies: ["float","xcolor", "colortbl"]
    toc: true
    toc_depth: 2
...
knitr::write_bib(c("TreeSummarizedExperiment", "scater", "mia", "miaViz", 
              "patchwork", "vegan", "ggplot2", "tidyverse", "ape", "phangorn", "msa", "miaQIIME2", "knitr", "kableExtra"), file = 'packages2.bib')
...
for (pkg in packages) {
  if (!requireNamespace(pkg, quietly = TRUE)) {
    if (pkg %in% c("TreeSummarizedExperiment", 
                   "scater", "mia", "miaViz", "miaQIIME2", 
                   "devtools", "rentrez")) {
...
```{r,out.width="85%", fig.cap=' Basic relative abundance of bacterial classes across all microbiome samples '}
...
```{r, out.width="85%", fig.cap=' Bacterial class composition by environment and anemone genotype (AIMS1 vs AIMS4, improved '}
...
```{r, out.width="85%", fig.cap=' Shannon alpha diversity boxplot stratified by anemone genotype '}
...
```{r, out.width="85%", fig.cap=' Shannon alpha diversity violinplot stratified by anemone genotype'}
...
```{r, out.width="85%", fig.cap=' Shannon alpha diversity boxplot across control vs sterile seawater conditions by environmental conditions'}
...
```{r, out.width="85%", fig.cap=' Shannon alpha diversity violinplot across control vs sterile seawater conditions by environmental conditions'}
#If needed, we can save it:
...
knitr::kable(p_value_summary, digits = 6,
...
```{r, out.width="85%", fig.cap=' Comparison Shannon alpha diversity violinplot across genotype AIMS1 vs AIMS4 '}
...
```{r, out.width="85%", fig.cap=' Comparison Shannon alpha diversity violinplot across control vs sterile seawater conditions by environmental conditions'}
...
\newpage
# Part 2: Protein Folding prediction
...
```{r,eval=FALSE}
# Enter your desired accession number. 
#In sequence, change nuccore=DNA/RNA;protein=protein sequence
...
```{r, echo=FALSE, out.width="85%", fig.cap=' ColabFold-predicted structures of fatty acid photodecarboxylase homologs with cysteine residues highlighted '}
knitr::include_graphics("Alignment_01Seq_02_Seq.png")
```

For this view, the following commands were used in the following order after opening the two structure predictions:

```
align 01sequence_input, 02sequence_input
bg_color white
show spheres, resn cys
select resn cys
color greencyan, resn cys

```
By using commands like show (spheres, dorts, cartoon, ribbon) and color (red, green, yellow, cyan), the design and representation for certain compounds like certain residues (resn ala/cys/phe for example) or also for pLDDT scores may be achieved.
